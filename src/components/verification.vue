                ...
                <div class="row">
                    <div class="col-6-lg" id="second-col">
                        ...
                        <div class="input-group">
                            ...
                            <input v-for="(digit, index) in otpDigits" :ref="`otpInput${index}`" :key="index"
                                v-model="otp[index]" type="text" min="0" max="9" @input="handleInput(index)"
                                @keydown.backspace="handleBackspace(index)" @paste="handlePaste($event, index)"
                                class="otp-input" />
                        </div>
                    </div>
                    ...
                    <router-link :to="{ path: '/setnewpassword' }" custom v-slot="{ navigate }">
                        <button class="submit-btn" type="btn" @click="navigate, callAPI()" role="link"
                            id="sub-btn">Verify</button>
                    </router-link>
                    <p id="p-2" :type="showtext ? 'text' : 'text'" v-show="!showtext || resendTimer === 0">
                        Didn't receive code? <span @click="resendCode" :disabled="resendDisabled"
                            v-on:click="toggletext()">Resend</span>
                    </p>
                    <p class="second-p" :type="showtext ? 'text' : 'text'" v-show="showtext" v-if="resendTimer > 0">
                        Code has been sent again! <span> Send again {{ Math.floor(resendTimer / 60) }}:{{ ("0" +
                            (resendTimer % 60)).slice(-2) }}</span>
                    </p>
                </div>
            </div>
        </div>
        <div class="col-lg-6 hidden sm:block" id="rigth-side">
